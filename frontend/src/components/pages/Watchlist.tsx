...
import {tickerApi} from "@/lib/api.ts";
...
export const Watchlist = () => {
    ...
    const handleAddStock = async (s: { stockSymbol: string; instrumentKey: string }) => {
        try {
            const t = await tickerApi.getLtpc([s.instrumentKey]);
            const liveData = t.data as any;
            const ltp = liveData[s.instrumentKey]?.ltp ?? null;
            if (!ltp || ltp <= 0) {
                // fallback — market closed, use a dummy price or block
                addToast('Live price unavailable. Try during market hours.', 'error');
                return;
            }
            const success = await addStock(s.stockSymbol, s.instrumentKey, Number(ltp));
            if (success) setAddSymbol('');
        } catch {
            addToast('Failed to fetch live price', 'error');
        }
    };

    useEffect(() => {
        const t = setTimeout(() => {
            searchStocks(addSymbol);
        }, 400);
        ...
        <div className="space-y-8 pb-12">
            ...
            {showCreate && (
                <div className="bg-neutral rounded-xl p-5 flex gap-3 items-end border border-border-light">
                    <div className="flex-1">
                        <label className="text-[9px] text-muted uppercase tracking-widest block mb-1.5">Watchlist
                            Name</label>
                    ...
                    <button onClick={handleCreate}
                            className="px-4 py-2.5 bg-accent text-white text-[12px] font-semibold hover:bg-accent/90 rounded-lg transition-colors">Create
                    </button>
                    <button onClick={() => setShowCreate(false)}
                            className="px-3 py-2.5 text-muted hover:text-primary rounded-lg hover:bg-neutral transition-colors">
            ...
            <div className="grid grid-cols-12 gap-6">
                ...
                <div className="col-span-4 space-y-2">
                    {lists.length === 0 && (
                        <p className="text-[13px] text-muted py-8 text-center">No watchlists yet. Create one to get
                            started.</p>
                    )}
                    {lists.map(w => (
                        <div key={w.watchlistId}>
                            <button onClick={() => loadDetail(w.watchlistId)}
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <button onClick={e => {
                                        e.stopPropagation();
                                        setDeleteConfirmId(w.watchlistId);
                                    }}
                                ...
                                <p className="text-[10px] text-muted mt-1">
                                    Created {new Date(w.createdAt).toLocaleDateString('en-IN', {
                                    day: 'numeric',
                                    month: 'short',
                                    year: 'numeric'
                                })}
                            ...
                            {deleteConfirmId === w.watchlistId && (
                                <div
                                    className="bg-negative/10 p-3 flex justify-between items-center border border-negative/30">
                    ...
                    <AlertTriangle className="h-3 w-3"/> Delete {w.watchlistName}?
                                    ...
                                    <div className="flex gap-2">
                                        <button onClick={() => setDeleteConfirmId(null)}
                                                className="text-[11px] px-2 py-1 bg-surface border border-negative/30 text-negative hover:bg-negative/20 transition-colors">Cancel
                                        </button>
                                        <button onClick={() => handleDelete(w.watchlistId)}
                                                className="text-[11px] px-2 py-1 bg-negative text-white hover:bg-negative/90 transition-colors">Delete
                                        </button>
                                    </div>
                                </div>
                            )}
                    ...
                    {activeId && !detailLoading && activeList && (
                        <div className="bg-surface rounded-xl border border-border-light p-5">
                            ...
                            {activeList.watchlistStocks.length === 0 ? (
                                <p className="text-[13px] text-muted py-8 text-center">No stocks in this watchlist. Use
                                    the search above to add.</p>
                            ) : (
                                        ...
                                        return (
                                            <tr key={s.stockSymbol}
                                                ...
                                                <td className="py-3 text-right">
                                                    {pChg != null ? (
                                                        <span
                                                            className={`text-[12px] font-medium px-1.5 py-0.5 rounded-sm ${
                                                                isUp
